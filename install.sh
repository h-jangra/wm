...
if [[ $FIX_DBUS -eq 1 || $FIX_AUDIO -eq 1 || $FIX_BLUETOOTH -eq 1 ]]; then
    if [[ $FIX_DBUS -eq 1 ]]; then
        "$SCRIPT_DIR/scripts/fix-dbus"
    fi
    if [[ $FIX_AUDIO -eq 1 ]]; then
        "$SCRIPT_DIR/scripts/fix-audio"
    fi
    if [[ $FIX_BLUETOOTH -eq 1 ]]; then
        "$SCRIPT_DIR/scripts/fix-bluetooth"
    fi
    exit 0
fi
...
if [[ -L "$HOME/.config/wm" && "$(readlink -f "$HOME/.config/wm" 2>/dev/null || true)" == "$(readlink -f "$SCRIPT_DIR" 2>/dev/null || true)" ]]; then
    msg_ok "Convenience link ~/.config/wm already points to $SCRIPT_DIR."
else
    ln -sfn "$SCRIPT_DIR" "$HOME/.config/wm"
fi

# Thunar xfconf configuration (~/.config/xfce4/xfconf/xfce-perchannel-xml/thunar.xml)
mkdir -p "$HOME/.config/xfce4/xfconf/xfce-perchannel-xml"
if [[ ! -f "$HOME/.config/xfce4/xfconf/xfce-perchannel-xml/thunar.xml" ]] || ! cmp -s "$SCRIPT_DIR/thunar/thunar.xml" "$HOME/.config/xfce4/xfconf/xfce-perchannel-xml/thunar.xml"; then
    cp "$SCRIPT_DIR/thunar/thunar.xml" "$HOME/.config/xfce4/xfconf/xfce-perchannel-xml/thunar.xml"
    msg_ok "Configured Thunar preferences (xfce4/thunar.xml)."
fi

# GTK 4.0 theme configuration (~/.config/gtk-4.0)
mkdir -p "$HOME/.config/gtk-4.0"
if [[ -f "$SCRIPT_DIR/gtk-3.0/gtk.css" ]]; then
    ln -sf "$SCRIPT_DIR/gtk-3.0/gtk.css" "$HOME/.config/gtk-4.0/gtk.css"
    ln -sf "$SCRIPT_DIR/gtk-3.0/settings.ini" "$HOME/.config/gtk-4.0/settings.ini"
...
if [[ -d "$SCRIPT_DIR/fonts" ]]; then
    for font_file in "$SCRIPT_DIR"/fonts/*.ttf "$SCRIPT_DIR"/fonts/*.otf; do
...
cp "$SCRIPT_DIR/services/mango.desktop" "$HOME/.local/share/wayland-sessions/mango.desktop"

if command -v sudo >/dev/null 2>&1 && [[ -f "$SCRIPT_DIR/services/mango.desktop" ]]; then
...
if [[ -f /usr/share/wayland-sessions/mango.desktop ]]; then
    ...
    if [[ "$desktop_cmd" == "dbus-run-session mangowc" ]]; then
        ...
        msg_info "To enforce 'dbus-run-session mangowc', run: sudo cp $SCRIPT_DIR/services/mango.desktop /usr/share/wayland-sessions/"
    fi
fi

# Initialize dynamic wallpaper theme palette
if [[ -x "$SCRIPT_DIR/scripts/generate-palette" && -f "$SCRIPT_DIR/wallpapers/nord.jpg" ]]; then
    if [[ ! -f "$SCRIPT_DIR/themes/palette.css" ]]; then
...
EOF
    ...
    if [[ -f /etc/ly/config.ini ]]; then
        if ! cmp -s "$SCRIPT_DIR/ly/config.ini" /etc/ly/config.ini; then
