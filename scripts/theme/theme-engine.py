...
MANGO_CONF = os.path.join(CONFIG_DIR, "mango", "config.conf")
MAKO_CONF = os.path.join(CONFIG_DIR, "mako", "config")
GTK_CSS = os.path.join(CONFIG_DIR, "gtk-3.0", "theme.css")
FOOT_CONF = os.path.join(CONFIG_DIR, "foot", "foot.ini")
ROFI_SHARED_PATH = os.path.join(CONFIG_DIR, "rofi", "shared.rasi")
EWW_COLORS_PATH = os.path.join(CONFIG_DIR, "eww", "styles", "_colors.scss")
MANGOBAR_STYLE_PATH = os.path.join(CONFIG_DIR, "mangobar", "theme.css")
...
def write_file(path, content, *, create_parent=True):
    """Write text to a file, optionally creating its parent directory."""
    if create_parent:
...
def find_theme_wallpaper(theme_id, preferred=""):
    ...
    if preferred:
        repository_wallpaper = os.path.join(
            REPO_DIR,
            "wallpapers",
            os.path.basename(preferred),
        )
        if os.path.isfile(repository_wallpaper):
            return repository_wallpaper
...
def generate_eww_scss(theme):
    bg = value(theme, "capsule_bg")
    fg = value(theme, "capsule_fg")
    border = value(theme, "border")
    accent = value(theme, "accent")
    accent_alt = value(theme, "accent_alt")
    surface = value(theme, "background")
    surface_hover = value(theme, "capsule_hover")
    muted = value(theme, "color8")

    rgb = tuple(int(bg[index:index + 2], 16) for index in (1, 3, 5))

    lines = [
        "// Generated by theme-engine for Eww",
        f"$bg: {bg};",
        f"$bg_trans: rgba({rgb[0]}, {rgb[1]}, {rgb[2]}, 0.92);",
        f"$surface: {surface};",
        f"$surface_hover: {surface_hover};",
        f"$surface_active: {border};",
        f"$border: {border};",
        "$border_subtle: rgba(255, 255, 255, 0.08);",
        f"$fg: {fg};",
        f"$fg_muted: {muted};",
        f"$fg_dim: {value(theme, 'color8', '#585b70')};",
        f"$accent: {accent};",
        f"$accent_alt: {accent_alt};",
        f"$success: {value(theme, 'vol')};",
        f"$warning: {value(theme, 'net')};",
        f"$danger: {value(theme, 'ws_urgent')};",
        f"$clock: {value(theme, 'clock')};",
        f"$ram: {value(theme, 'ram')};",
        f"$net: {value(theme, 'net')};",
        f"$bat: {value(theme, 'bat')};",
        f"$vol: {value(theme, 'vol')};",
        f"$bt: {value(theme, 'bt')};",
        f"$ws_focused: {value(theme, 'ws_focused')};",
        f"$ws_occupied: {value(theme, 'ws_occupied')};",
        f"$ws_empty: {value(theme, 'ws_empty')};",
        "$border_rad: 10px;",
    ]
...
def generate_mangobar_css(theme):
    return f"""/* =============
 * Autogenerated Theme Colors
 * ============= */

@define-color bg {value(theme, 'background')};
...
def update_wallpaper_links(theme_id):
    """Update the theme and current wallpaper directory symlinks."""
    wallpaper_dir = os.path.join(PRESETS_DIR, theme_id, "wallpapers")
    if not os.path.isdir(wallpaper_dir):
        return

    wallpaper_root = os.path.join(REPO_DIR, "wallpapers")
    relative_target = os.path.relpath(wallpaper_dir, wallpaper_root)

    for link_name in ("theme", "current"):
        link_path = os.path.join(wallpaper_root, link_name)
        ...
        try:
            ...
            print(
                f"Warning: could not create {link_path}: {error}",
                file=sys.stderr,
            )
...
def switch_wallpaper(theme_id, theme):
    ...
    wallpaper_manager = first_existing_file(
        (
            os.path.join(SCRIPTS_DIR, "wallpaper", "wallpaper-manager"),
            os.path.join(SCRIPTS_DIR, "wallpaper-manager"),
        )
    )
...
def apply_theme(theme_id, set_wallpaper=True):
    ...
    eww_colors = "\n".join(
        [
            f"$background: {value(theme, 'capsule_bg', theme.get('background', '#121212'))};",
            f"$foreground: {value(theme, 'capsule_fg', theme.get('foreground', '#f5f5f5'))};",
            f"$accent: {value(theme, 'accent', '#f5f5f5')};",
            "",
        ]
    )

    write_file(EWW_COLORS_PATH, eww_colors)
    write_file(
        os.path.join(GENERATED_DIR, "palette.scss"),
        generate_eww_scss(theme),
    )

    # Rofi
    rofi_tokens = generate_rofi_shared_rasi(theme)
    write_file(ROFI_SHARED_PATH, rofi_tokens)
    write_file(
    ...
    write_file(
        os.path.join(CONFIG_DIR, "foot", "colors.ini"),
        foot_config,
    )
